...
def load_best_model(model_name: str, dirs: dict) -> nn.Module:
    ...
    if not os.path.exists(best_path):
        raise FileNotFoundError(f"Checkpoint tidak ditemukan: {best_path}\nJalankan train.py terlebih dahulu untuk model '{model_name}'.")
...
def _save_comparison_table(all_results: dict):
    ...
    pdf_path = os.path.join(COMPARISON_DIR, "laporan_evaluasi_lengkap.pdf")
    
    try:
        with PdfPages(pdf_path) as pdf:
            fig_table, ax_table = plt.subplots(figsize=(18, 5))
            ax_table.axis('off')
            columns = ["Model", "Test Acc", "Val Acc\n(Best/Avg/Low)", "Val F1\n(Best/Avg/Low)", "Inf. Time\n(ms)", "VRAM\n(MB)", "FLOPs", "Params\n(M)"]
            cell_text = []
            ...
            plt.title("Tabel Metrik Performa Keseluruhan", fontsize=16, pad=20, fontweight='bold')
            ...
            axes[0, 0].bar(model_names, test_accuracies, color=["#4C72B0", "#55A868", "#C44E52"])
            axes[0, 0].set_title("Test Accuracy (%)")
            axes[0, 0].grid(axis="y", alpha=0.3)
            axes[0, 1].bar(model_names, test_f1s, color=["#4C72B0", "#55A868", "#C44E52"])
            axes[0, 1].set_title("Test F1-Score (Macro)")
            axes[0, 1].grid(axis="y", alpha=0.3)
            axes[1, 0].bar(model_names, inf_times, color=["#4C72B0", "#55A868", "#C44E52"])
            axes[1, 0].set_title("Inference Time (ms/gambar)")
            axes[1, 0].grid(axis="y", alpha=0.3)
            axes[1, 1].bar(model_names, vram_usages, color=["#4C72B0", "#55A868", "#C44E52"])
            axes[1, 1].set_title("GPU VRAM Peak Usage (MB)")
            axes[1, 1].grid(axis="y", alpha=0.3)
            plt.suptitle("Grafik Perbandingan: ResNet50 vs MobileNetV3 vs ViT", fontsize=16, fontweight='bold')
            plt.tight_layout()
            ...
            for m_name, res in all_results.items():
                ...
                axes_curve[0].plot(epochs, v_acc, label="Validation Accuracy", color='#4C72B0', marker='o', markersize=4, linewidth=1.5)
                axes_curve[0].plot(epochs, t_acc, label="Training Accuracy", color='#C44E52', marker='x', markersize=4, linewidth=1.5)
                ...
                axes_curve[1].plot(epochs, v_err, label="Validation_Error", color='#4C72B0', marker='o', markersize=4, linewidth=1.5)
                axes_curve[1].plot(epochs, t_err, label="Training_Error", color='#C44E52', marker='x', markersize=4, linewidth=1.5)
                axes_curve[1].set_title("Error_Rate", fontsize=14)
                ...
                plt.suptitle(f"Learning Curves with Epoch = {len(epochs)} ({res['display_name']})", fontsize=15, fontweight='bold')
                plt.subplots_adjust(bottom=0.25)
                pdf.savefig(fig_curve, bbox_inches='tight')
                plt.close(fig_curve)
                
        print(f"\n[SUKSES] Laporan PDF komprehensif berhasil dibuat → {pdf_path}")
    except Exception as e:
        print(f"\n[WARNING] Gagal membuat laporan PDF: {e}")
